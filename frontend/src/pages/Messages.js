...
const individualContacts = Object.keys(
  conversations.conversations.individual
).map((name, index) => ({
  id: index + 1,
  name,
  avatar: "https://via.placeholder.com/40",
  lastMessage:
    conversations.conversations.individual[name].slice(-1)[0].content,
  isPinned: false,
  isOnline: Math.random() > 0.5,
}));

const groupChats = Object.keys(conversations.conversations.group).map(
  (name, index) => ({
...
const Messages = () => {
  ...
  const [messages, setMessages] = useState(
    conversations.conversations.individual[selectedContact.name] || []
  );
  const [newMessage, setNewMessage] = useState("");
  const [pinnedContacts, setPinnedContacts] = useState(
    contacts.filter((contact) => contact.isPinned)
  );
  ...
  const handleSendMessage = () => {
    if (newMessage.trim()) {
      const updatedMessages = [
        ...messages,
        { user: "You", content: newMessage, timestamp: "Now" },
      ];
  ...
  const handleFileUpload = (event) => {
    ...
    if (file) {
      const updatedMessages = [
        ...
        {
          user: "You",
          content: `Sent a photo: ${file.name}`,
          timestamp: "Now",
          isPhoto: true,
        },
      ];
    ...
    if (query.trim()) {
      const results = contacts.filter(
        (contact) =>
          ...
          initialMessages[contact.id]?.some((msg) =>
            ...
            >
              {messages.map((msg, index) => (
                ...
                >
                  ...
                  <p className="text-xs mt-1 text-gray-300">{msg.timestamp}</p>{" "}
                  {/* Timestamp positioned outside */}
                </div>
              ))}
            </div>
