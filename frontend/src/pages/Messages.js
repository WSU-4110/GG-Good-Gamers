...
const handleMenuClose = () => {
  ...
  const deleteContact = async (contact) => {
    try {
      ...
      const usersRef = collection(db, "users");
      const userDocQuery = query(usersRef, where("email", "==", currentUser.email));
...
<div>
  ...
  <ul className="space-y-4">
    {filteredContacts
      .filter((contact) => !pinnedContacts.includes(contact))
      .map((contact) => (
        <li
          key={contact}
          className={`p-4 rounded-lg flex items-center justify-between ${
...
>
  <DialogTitle style={{ textAlign: "center", fontWeight: "bold", color: "white" }}>
    Add a Friend
  </DialogTitle>
  <DialogContent>
    <TextField
    ...
    <List>
      {globalSearchResults.map((user) => (
        ...
        >
          <ListItemAvatar>
            <Avatar sx={{ backgroundColor: "#6b7280" }} /> {/* Match avatar to theme */}
          </ListItemAvatar>
          <ListItemText
            primary={user}
            style={{
          ...
          <Button
            ...
            onMouseOver={(e) => (e.target.style.backgroundColor = "#3b82f6")} // Blue hover
            onMouseOut={(e) => (e.target.style.backgroundColor = "#4f46e5")} // Revert hover
          >
            Add
          </Button>
        </ListItem>
